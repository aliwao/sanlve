...
final class template_cache {
	...
	public function template_compile($module, $template, $style = 'default') {
		...
		$filepath = SANLVE_PATH.'caches/caches_template/'.$module.'/';
	    if(!is_dir($filepath)) {
	...
	public function template_compile_admin($module, $template) {
		...
		$content = @file_get_contents ( $tplfile );
		$filepath = SANLVE_PATH.'caches/caches_template/admintpl/'.$module.'/';
	    if(!is_dir($filepath)) {
			mkdir($filepath, 0777, true);
	    }
	...
	public function template_parse($str, $istag = 0) {
		...
		if (! $istag)
			$str = "<?php defined('IN_SANLVE') or exit('No permission resources.'); ?>" . $str;
		return $str;
	}
