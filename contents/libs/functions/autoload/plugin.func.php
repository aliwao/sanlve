...
function plugin_stat($appid = '') {
	...
	$url = 'http://open.contents.cn/api.php?op=appstatus&'.$data.'&appid='.$appid;
...
function plugin_install_stat($appid){
	...
	$url = 'http://open.contents.cn/api.php?op=appstatus&isinstall=1';
...
function app_validity_check($appid) {
	...
	$appid = intval($appid);
	$url = $header = '';
	$url = 'http://open.contents.cn/api.php?op=appstatus';
	$headers = get_headers($url.'&appid='.$appid,1);
	$status = $headers['pc_appstatus'];
	return $status;
}
