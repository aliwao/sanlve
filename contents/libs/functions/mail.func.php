...
function sendmail($toemail, $subject, $message, $from='',$cfg = array(), $sitename='') {
	...
	$headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: SANLVE-V9 {$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/html; charset=".CHARSET."{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
	...
	fputs($fp, ($mail['auth'] ? 'EHLO' : 'HELO')." contents\r\n");
	$lastmessage = fgets($fp, 512);
