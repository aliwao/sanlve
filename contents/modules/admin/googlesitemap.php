...
class googlesitemap extends admin {
	...
	function set () {
		...
		//根据当前站点,取得文件存放路径
 		...
 		//判断当前站点目录,是SANLVE则把文件写到根目录下, 不是则写到分站目录下.(分站目录用由静态文件路经html_root和分站目录dirname组成)
 		if($siteid==1){
 			$dir = SL_PATH;
 		}else {
 			$dir = SL_PATH.$html_root.DIRECTORY_SEPARATOR.$sitecache[$siteid]['dirname'].DIRECTORY_SEPARATOR;
 		}
 		//模型缓存
