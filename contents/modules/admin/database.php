...
class database extends admin {
		...
		if($_GET['dosubmit']) {
			...
			if(is_array($sqlfiles)) {
				...
				foreach($sqlfiles as $id=>$sqlfile) {
					//老的数据库备份文件转换为新格式
					if(preg_match("/([contentstables_|db_][0-9]{8}_[0-9a-z]{20}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
					...
					if(preg_match("/([0-9a-z]{20}_[contentstables_|db_]+[0-9]{8}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
	...
	private function status($tables,$tablepre) {
		...
		return array('contentstables'=>$contents, 'othertables'=>$other);
	}
	 ...
	 * @param unknown_type $tabletype 备份数据库类型 （非contents数据与contents数据）
	 */
	 ...
	 */
 	...
 	private function sql_split($sql) {
		...
		if($this->db_tablepre != "contents_") $sql = str_replace("`contents_", '`'.$this->db_tablepre, $sql);
		$sql = str_replace("\r", "\n", $sql);
		$ret = array();
		$num = 0;
