...
class plugin extends admin {
	...
	public function appcenter() {
		...
		$data = file_get_contents('http://open.contents.cn/index.php?m=open&c=api&a=get_applist&s='.$s.'&p='.$p);
		$data = array_iconv(json_decode($data, true),'utf-8',CHARSET);
		
		$recommed_data = file_get_contents('http://open.contents.cn/index.php?m=open&c=api&a=get_recommed_applist&s=5&p=1');
		$recommed_data = array_iconv(json_decode($recommed_data, true),'utf-8',CHARSET);
		
		$focus_data = file_get_contents('http://open.contents.cn/index.php?m=open&c=api&a=get_app_focus&num=3');
	...
	public function appcenter_detail() {
		$data = array();
		$id = intval($_GET['id']);
		$data = file_get_contents('http://open.contents.cn/index.php?m=open&c=api&a=get_detail_byappid&id='.$id);
		$data = array_iconv(json_decode($data, true),'utf-8',CHARSET);
		extract($data);		
		if($appname) {
...
EOF;
			...
			//静态程序路径
			$copy_to_staticspath = SL_PATH.'statics'.DIRECTORY_SEPARATOR.'plugin';

			//应用文件夹名称
			$appdirname = $data['appenname'];
	
			$this->copyfailnum = 0;
			$this->copydir($copy_from.DIRECTORY_SEPARATOR.'contents'.DIRECTORY_SEPARATOR.'plugin', $copy_to_pcpath, $_GET['cover']);
