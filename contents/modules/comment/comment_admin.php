...
class comment_admin extends admin {
	...
	public function listinfo() {
		...
		if (isset($_GET['search'])) {
			...
			switch ($searchtype) {
				case '0':
					$sql = "SELECT `commentid` FROM `contents_comment` WHERE `siteid` = '$this->siteid' AND `title` LIKE '%$keywords%' AND `tableid` = '$tableid' ";
				...
				case '1':
					...
					$sql = "SELECT `commentid` FROM `contents_comment` WHERE `commentid` LIKE 'content_%-$keywords-%' ";
					$this->comment_db->query($sql);
					$data = $this->comment_db->fetch_array();
					if (!empty($data)) {
						foreach ($data as $d) {
