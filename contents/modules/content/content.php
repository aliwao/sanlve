						...
						if ($t=='content') {
							...
							foreach ($model as $modelid) {
								...
								if ($r = $db->count()) { //判断模型下是否有数据
									...
									$db->query('TRUNCATE TABLE `contents_'.$model_arr[$modelid]['tablename'].'`');
									//开始清理模型data表数据
									...
									$db->query('TRUNCATE TABLE `contents_'.$model_arr[$modelid]['tablename'].'_data`');
									//删除该模型中在hits表的数据
							...
							for($i=1;;$i++) {
								...
								if ($comment_db->table_exists(str_replace($comment_db->db_tablepre, '', $comment_db->table_name))) {
									if ($r = $comment_db->count()) {
										...
										$comment_db->query('TRUNCATE TABLE `contents_comment_data_'.$i.'`');
									}
								} else {
									break;
								}
							...
							if ($r = $db->count()) {
								...
								$db->query('TRUNCATE TABLE `contents_'.$t.'`');
							}
						}
					}
				}
