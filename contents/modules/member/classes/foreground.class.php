	...
	final public function check_member() {
		...
		if(ROUTE_M =='member' && ROUTE_C =='index' && in_array(ROUTE_A, array('login', 'register', 'mini','send_newmail'))) {
			...
			if ($contents_auth) {
				...
				list($userid, $password) = explode("\t", sys_auth($contents_auth, 'DECODE', $auth_key));
				//验证用户，获取用户信息
				...
				unset($userid, $password, $contents_auth, $auth_key);
			} else {
