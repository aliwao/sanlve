...
class index extends foreground {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				if($status > 0) {
					...
					//传入phpsso为明文密码，加密后存入contents_v9
					...
					if($userid > 0) {
						...
						if($userinfo['groupid'] == 7) {
							...
							$contents_auth = sys_auth($userid."\t".$userinfo['password'], 'ENCODE', get_auth_key('login'));
							
							param::set_cookie('auth', $contents_auth, $cookietime);
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$contents_auth = sys_auth($userid."\t".$password, 'ENCODE', get_auth_key('login'));
	...
	/**
	*通过手机修改密码
	*方式：用户发送HHPWD afei985#821008 至 1065788 ，SANLVE进行转发到网站运营者指定的回调地址，在回调地址程序进行密码修改等操作,处理成功时给用户发条短信确认。
	*contents 以POST方式传递相关数据到回调程序中
